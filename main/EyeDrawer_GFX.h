...
class EyeDrawer {
  public:
    static void Draw(MatrixPanel_I2S_DMA *dma_display, int16_t centerX, int16_t centerY, EyeConfig *config) {
      ...
      // Fill eye centre
      EyeDrawer::FillRectangle(dma_display, min_c_x, min_c_y, max_c_x, max_c_y, 1);

      // Fill eye outwards to meet edges of rounded corners 
      EyeDrawer::FillRectangle(dma_display, TRc_x, TRc_y, BRc_x + config->Radius_Bottom, BRc_y, 1); // Right
		  EyeDrawer::FillRectangle(dma_display, TLc_x - config->Radius_Top, TLc_y, BLc_x, BLc_y, 1); // Left
		  EyeDrawer::FillRectangle(dma_display, TLc_x, TLc_y - config->Radius_Top, TRc_x, TRc_y, 1); // Top
		  EyeDrawer::FillRectangle(dma_display, BLc_x, BLc_y, BRc_x, BRc_y + config->Radius_Bottom, 1); // Bottom
      ...
      if(config->Slope_Top > 0) {
        EyeDrawer::FillRectangularTriangle(dma_display, TLc_x, TLc_y-config->Radius_Top, TRc_x, TRc_y-config->Radius_Top, 0);
        EyeDrawer::FillRectangularTriangle(dma_display, TRc_x, TRc_y-config->Radius_Top, TLc_x, TLc_y-config->Radius_Top, 1);
      } 
      else if(config->Slope_Top < 0) {
        EyeDrawer::FillRectangularTriangle(dma_display, TRc_x, TRc_y-config->Radius_Top, TLc_x, TLc_y-config->Radius_Top, 0);
        EyeDrawer::FillRectangularTriangle(dma_display, TLc_x, TLc_y-config->Radius_Top, TRc_x, TRc_y-config->Radius_Top, 1);
      }
      // Draw slanted edges at bottom of eyes
      if(config->Slope_Bottom > 0) {
        EyeDrawer::FillRectangularTriangle(dma_display, BRc_x+config->Radius_Bottom, BRc_y+config->Radius_Bottom, BLc_x-config->Radius_Bottom, BLc_y+config->Radius_Bottom, 0);
        EyeDrawer::FillRectangularTriangle(dma_display, BLc_x-config->Radius_Bottom, BLc_y+config->Radius_Bottom, BRc_x+config->Radius_Bottom, BRc_y+config->Radius_Bottom, 1);
      }
      else if (config->Slope_Bottom < 0) {
        EyeDrawer::FillRectangularTriangle(dma_display, BLc_x-config->Radius_Bottom, BLc_y+config->Radius_Bottom, BRc_x+config->Radius_Bottom, BRc_y+config->Radius_Bottom, 0);
        EyeDrawer::FillRectangularTriangle(dma_display, BRc_x+config->Radius_Bottom, BRc_y+config->Radius_Bottom, BLc_x-config->Radius_Bottom, BLc_y+config->Radius_Bottom, 1);
      }

      // Draw corners (which extend "outwards" towards corner of screen from supplied coordinate values)
      if(config->Radius_Top > 0) {
        EyeDrawer::FillEllipseCorner(dma_display, T_L, TLc_x, TLc_y, config->Radius_Top, config->Radius_Top, 1);
        EyeDrawer::FillEllipseCorner(dma_display, T_R, TRc_x, TRc_y, config->Radius_Top, config->Radius_Top, 1);
      }
      if(config->Radius_Bottom > 0) {
        EyeDrawer::FillEllipseCorner(dma_display, B_L, BLc_x, BLc_y, config->Radius_Bottom, config->Radius_Bottom, 1);
        EyeDrawer::FillEllipseCorner(dma_display, B_R, BRc_x, BRc_y, config->Radius_Bottom, config->Radius_Bottom, 1);
      }
    }
    ...
    static void FillEllipseCorner(MatrixPanel_I2S_DMA *dma_display, CornerType corner, int16_t x0, int16_t y0, int32_t rx, int32_t ry, uint16_t color) {
      ...
      if (corner == T_R) {
        for(x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
          dma_display->writeFastHLine(x0, y0 - y, x, 63781);
          if(s >= 0) {
            s += fx2 * (1 - y);
            y--;
          }
        ...
        for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
          dma_display->writeFastHLine(x0, y0 + y -1, x, 63781);
          if (s >= 0) {
            s += fx2 * (1 - y);
            y--;
          }
        ...
        for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
          dma_display->writeFastHLine(x0-x, y0 - y, x, 63781);
          if (s >= 0) {
            s += fx2 * (1 - y);
            y--;
          }
        ...
        for (x = 0, y = ry, s = 2 * ry2 + rx2 * (1 - 2 * ry); ry2 * x <= rx2 * y; x++) {
          dma_display->writeFastHLine(x0-x, y0 + y - 1, x, 63781);
          if (s >= 0) {
            s += fx2 * (1 - y);
            y--;
          }
    ...
    static void FillRectangle(MatrixPanel_I2S_DMA *dma_display, int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t color) {
      ...
      int32_t w = r-l;
      int32_t h = b-t; 
      dma_display->drawRect(l,t,w,h,color);
    //   u8g2.setDrawColor(color);
    //   u8g2.drawBox(l, t, w, h);
    //   u8g2.setDrawColor(1);
    }

    static void FillRectangularTriangle(MatrixPanel_I2S_DMA *dma_display, int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t color) {
    //   u8g2.setDrawColor(color);
    //   u8g2.drawTriangle(x0, y0, x1, y1, x1, y0);
    //   u8g2.setDrawColor(1);
        dma_display->drawTriangle(x0, y0, x1, y1, x1, y0, color);
    }

    static void FillTriangle(MatrixPanel_I2S_DMA *dma_display, int32_t x0, int32_t y0, int32_t x1, int32_t y1, int32_t x2, int32_t y2, int32_t color) {
        // u8g2.setDrawColor(color);
        // u8g2.drawTriangle(x0, y0, x1, y1, x2, y2);
        // u8g2.setDrawColor(1);
        dma_display->drawTriangle(x0, y0, x1, y1, x2, y2, color);
    }
};
